...
\keywords{second principe, maximum d'entropie, système isolé, paroi diatherme, paroi mobile, équilibre thermique, équilibre mécanique}

Un récipient rigide et isolé de l'extérieur contient deux gaz parfaits
monoatomiques, séparés par une paroi interne initialement fixe, étanche et
adiabatique. Le compartiment 1 contient $n_1$ moles à la température $T_1$ ;
le compartiment 2 contient $n_2=3n_1$ moles à la température $T_2=T_1/2$.
Les deux compartiments ont initialement le même volume $V$. On pose
$P_0=n_1RT_1/V$.

\begin{enumerate}
\item Faire un schéma simple du récipient. Indiquer les deux gaz, la paroi
interne, les quantités de matière, les températures, les volumes et les
pressions initiales.
\item Exprimer formellement les deux pressions initiales en fonction de $P_0$.
\item La paroi devient diatherme, mais reste fixe et étanche. À l'aide des deux
principes, justifier la condition d'équilibre thermique, puis déterminer la
température et les pressions finales.
\item Calculer l'énergie échangée entre les gaz et préciser sa forme. Établir
le bilan entropique du système global et vérifier que l'entropie créée est
positive.
\item La paroi devient maintenant diatherme et mobile. En maximisant l'entropie
du système isolé, justifier les conditions d'équilibre thermique et mécanique.
Déterminer la température, la pression et les volumes finaux, puis effectuer
le bilan entropique. Comparer avec le cas de la paroi fixe.
\item Application numérique pour $n_1=1\ \mathrm{mol}$,
$T_1=600\ \mathrm{K}$ et
$R=8{,}314\ \mathrm{J\,mol^{-1}\,K^{-1}}$.
\end{enumerate}

\begin{indication}
Le système global est isolé : son énergie interne est conservée et son
entropie est maximale à l'équilibre. Pour un gaz parfait,
\[
...
\begin{solution}
\textbf{Question 1.} Le schéma représente un récipient rigide de volume total
$2V$, partagé par une paroi. À gauche : $(n_1,T_1,V,P_0)$ ; à droite :
$(3n_1,T_1/2,V,P_{2,i})$. Les parois extérieures sont adiabatiques.

\textbf{Question 2 --- Calcul formel.} L'équation des gaz parfaits donne
\[
P_{1,i}=\frac{n_1RT_1}{V}=P_0,
\qquad
P_{2,i}=\frac{3n_1R(T_1/2)}{V}=\frac32P_0.
\]

\textbf{Question 3 --- Paroi fixe et diatherme.} Lors d'un petit transfert
d'énergie avec $\mathrm{d}U_2=-\mathrm{d}U_1$, les volumes restent constants :
\[
\mathrm{d}S_{\mathrm{tot}}
=\left(\frac1{T_1}-\frac1{T_2}\right)\mathrm{d}U_1.
\]
À l'équilibre, l'entropie est maximale ; le coefficient de
$\mathrm{d}U_1$ doit donc s'annuler, ce qui impose
$T_{1,f}=T_{2,f}=T_f$. La conservation de l'énergie donne
\[
n_1\frac32R(T_f-T_1)
+3n_1\frac32R\left(T_f-\frac{T_1}{2}\right)=0,
\]
d'où
\[
T_f=\frac58T_1,
...
P_{1,f}=\frac58P_0,
\qquad
P_{2,f}=\frac{15}{8}P_0.
\]
La paroi étant fixe, l'équilibre mécanique n'est pas requis : les pressions
finales peuvent être différentes.

\textbf{Question 4 --- Bilan entropique.} Le gaz 2 reçoit uniquement sous
forme de chaleur l'énergie
\[
Q_{1\to2}=\Delta U_2
=3n_1\frac32R\left(\frac58T_1-\frac12T_1\right)
=\frac9{16}n_1RT_1.
\]
Le système global est isolé, donc $S_e=0$ et $S_c=\Delta S_{\mathrm{tot}}$.
Comme les volumes sont constants,
\[
S_c
=\frac32n_1R\left[
\ln\!\left(\frac58\right)
+3\ln\!\left(\frac54\right)
\right]>0.
\]
Cette création d'entropie traduit le transfert spontané de chaleur entre deux
gaz initialement à des températures différentes.

\textbf{Question 5 --- Paroi mobile et diatherme.} Avec
$\mathrm{d}U_2=-\mathrm{d}U_1$ et $\mathrm{d}V_2=-\mathrm{d}V_1$,
\[
\mathrm{d}S_{\mathrm{tot}}
=\left(\frac1{T_1}-\frac1{T_2}\right)\mathrm{d}U_1
+\left(\frac{P_1}{T_1}-\frac{P_2}{T_2}\right)\mathrm{d}V_1.
\]
La maximisation de l'entropie impose donc $T_{1,f}=T_{2,f}$ puis
$P_{1,f}=P_{2,f}$. La conservation de l'énergie donne encore
$T_f=5T_1/8$. À pression et température communes, les volumes sont
proportionnels aux quantités de matière. Comme leur somme vaut $2V$,
\[
V_{1,f}=\frac V2,
\qquad
V_{2,f}=\frac{3V}{2},
...
P_f=\frac54P_0.
\]
Les variations d'énergie interne sont les mêmes que pour la paroi fixe, mais
l'énergie s'échange ici sous forme de chaleur et de travail. Le bilan
entropique vaut
\[
S_c=\Delta S_{\mathrm{tot}}
=\frac32n_1R\left[
\ln\!\left(\frac58\right)+3\ln\!\left(\frac54\right)
\right]
+n_1R\left[
\ln\!\left(\frac12\right)+3\ln\!\left(\frac32\right)
\right]>0.
\]

\textbf{Question 6 --- Application numérique.} On obtient
\[
T_f=375\ \mathrm{K},
\qquad
Q_{1\to2}\simeq2{,}81\ \mathrm{kJ}.
\]
Pour la paroi fixe,
\[
P_{1,f}=0{,}625P_0,
...
P_{2,f}=1{,}875P_0,
\qquad
S_c\simeq2{,}49\ \mathrm{J\,K^{-1}}.
\]
Pour la paroi mobile,
\[
P_f=1{,}25P_0,
\qquad
V_{1,f}=0{,}50V,
...
\keywords{création d'entropie, compression adiabatique, irréversibilité, isentropique, bilan entropique, comparaison formelle}

On reprend les états de l'exercice \textit{Compression adiabatique :
progressive ou brutale} de la leçon 5. On pose
\[
x=\frac{P_1}{P_0}>1,
\qquad
...
\]
La compression progressive conduit de $A$ à $B$. La compression brutale
conduit de $A$ à $B'$, puis le retrait brutal de la surcharge conduit de $B'$
à $B''$. On rappelle
\[
\frac{T_{B'}}{T_A}=\theta,
\qquad
\frac{V_{B'}}{V_A}=\frac{\theta}{x},
\qquad
\frac{T_{B''}}{T_A}
=\frac{V_{B''}}{V_A}
=\theta\phi>1.
...
\begin{enumerate}
\item Calculer la variation d'entropie sur la compression réversible
$A\to B$.
\item Calculer formellement $\Delta S_{AB'}$. Montrer qu'elle est strictement
positive et expliquer pourquoi elle est égale à l'entropie créée.
\item Faire de même pour le retrait brutal $B'\to B''$.
\item Calculer directement la variation d'entropie entre $A$ et $B''$ et
vérifier qu'elle est égale à la somme des deux créations précédentes.
\item Comparer les bilans énergétiques et entropiques des deux expériences.
\end{enumerate}

\begin{indication}
Pour un gaz parfait à capacités thermiques constantes,
\[
...
R=(\gamma-1)C_V.
\]
Toutes les transformations sont adiabatiques : l'entropie échangée est nulle.
\end{indication}

\begin{solution}
\textbf{Question 1.} La transformation $A\to B$ est adiabatique et
réversible :
\[
\Delta S_{AB}=0.
\]
On retrouve ce résultat en injectant
$T_B/T_A=x^{(\gamma-1)/\gamma}$ et $V_B/V_A=x^{-1/\gamma}$ dans la formule
de $\Delta S$.

\textbf{Question 2.} Pour la compression brutale,
\[
\Delta S_{AB'}
=nC_V\ln\theta+nR\ln\frac{\theta}{x}
=nC_V\ln\left(\frac{\theta^\gamma}{x^{\gamma-1}}\right).
\]
L'inégalité des moyennes établie à la leçon 5 donne
$\theta>x^{(\gamma-1)/\gamma}$, donc
\[
\Delta S_{AB'}>0.
\]
Comme $Q=0$, l'entropie échangée est nulle :
$S_{c,AB'}=\Delta S_{AB'}>0$.

\textbf{Question 3.} Sur le retour brutal,
\[
\frac{T_{B''}}{T_{B'}}=\phi,
\qquad
\frac{V_{B''}}{V_{B'}}=x\phi.
\]
Ainsi,
\[
\Delta S_{B'B''}
=nC_V\ln\phi+nR\ln(x\phi)
=nC_V\ln\left(\phi^\gamma x^{\gamma-1}\right).
\]
La moyenne arithmétique pondérée
$\phi=[1+(\gamma-1)/x]/\gamma$ est strictement supérieure à la moyenne
géométrique pondérée $x^{-(\gamma-1)/\gamma}$. Par conséquent,
\[
\Delta S_{B'B''}=S_{c,B'B''}>0.
\]

\textbf{Question 4.} Les états $A$ et $B''$ ont la même pression et vérifient
$T_{B''}/T_A=V_{B''}/V_A=\theta\phi$. Le calcul direct donne
\[
\Delta S_{AB''}
=n(C_V+R)\ln(\theta\phi)
=nC_P\ln(\theta\phi)>0.
\]
Comme l'entropie est une fonction d'état,
\[
\Delta S_{AB''}
=\Delta S_{AB'}+\Delta S_{B'B''}.
\]
Toute cette augmentation est de l'entropie créée.

\textbf{Question 5.} Pour $A\to B$, le gaz reçoit du travail et augmente son
énergie interne, mais ne crée pas d'entropie. Pour
$A\to B'\to B''$, il reçoit un travail net
\[
W_{\mathrm{tot}}=\Delta U_{\mathrm{tot}}
=nC_VT_A(\theta\phi-1)>0
\]
et crée de l'entropie à chacune des deux étapes. L'état final $B''$ reste plus
chaud, plus volumineux et plus entropique que $A$ : le second principe
quantifie ainsi l'irréversibilité que le premier principe seul avait seulement
mise en évidence.
\end{solution}
\end{exo}
...
\begin{enumerate}
\item Montrer que l'état final est le même que la surcharge soit posée progressivement ou brutalement : $(P_1,\ V_4 = V_0/1{,}5 \approx 8{,}3$ L$,\ T_0)$. En déduire $\Delta S_{\text{gaz}}$, identique dans les deux cas.
